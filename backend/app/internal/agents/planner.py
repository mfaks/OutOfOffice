...
# Enumerate and score PTO windows for the year and return the top candidates
async def planner_node(state: TripState) -> dict:
    ...
    if priority == TripPriority.most_pto:
        ...
        # both best_yield and lowest_cost presort by yield here; lowest_cost is resorted by the ranker
        windows.sort(key=lambda w: w["yield_score"], reverse=True)

    return {"candidate_windows": windows[:5]}


# Walk forward day by day until the PTO budget is spent and return the window with its yield score
def _score_window(
    start: date,
    pto_budget: int,
    holidays: set[str],
